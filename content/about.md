---
title: "About"
date: 2024-01-18T21:38:52+08:00
lastmod: 2026-08-05T00:00:00+08:00
draft: false
keywords: ["Kai Zheng", "AI4Science", "Weather Forecasting"]
description: "About Kai Zheng - Algorithm Engineer focusing on AI for Science"
tags: ["About"]
categories: ["About"]
author: "kaizheng"
---

Hi, I'm **Kai Zheng**, an Algorithm Engineer passionate about **AI for Science**. My work spans large model development (code intelligence, LLM data flywheel), video prediction foundation models, multimodal generation, and weather/climate AI.

## Research Interests

- 🌦️ **Weather Forecasting Foundation Models** — data-driven medium-range forecasting, nowcasting, and super-resolution
- 🎥 **Video Prediction & Multimodal Generation** — diffusion models, generative models
- 🧠 **Transformer & Large Models** — foundation model architectures, RL post-training (PPO/GRPO/DAPO)
- 🔬 **AI4Science** — physics-informed deep learning, climate data assimilation

## Projects

- 📚 [Awesome-Weather-Forecast](https://github.com/zhengkai15/Awesome-Weather-Forecast) — curated list of AI weather forecasting research
- 🚀 Precipitation Super-Resolution with Diffusion
- 🌱 Data-Driven Climate Data Assimilation
- 🚀 Cryosphere Large Language Model
- 🐱 Image and Video Generation

## Experience

- 🧠 Large Model Development — code intelligence & data flywheel
- 🌦️ Senior Algorithm Researcher — video prediction foundation models, multimodal generation, AI4Science
- ⚡ Algorithm Engineer — time-series forecasting, renewable energy & power trading

***REMOVED***

***REMOVED***
***REMOVED***

## Contact

- 📧 Email: kaizheng.work@gmail.com
- 🐙 GitHub: [zhengkai15](https://github.com/zhengkai15)
